...
// utility functions
static inline jfloat db2lin(jfloat db){ // dB to linear
	return powf(10.0f, 0.05f * db);
}

static inline jint clampi(jint v, jint min, jint max){
	return v < min ? min : (v > max ? max : v);
}

static inline jfloat clampf(jfloat v, jfloat min, jfloat max){
	return v < min ? min : (v > max ? max : v);
}

static jboolean isprime(jint v){
	if (v < 0)
		return isprime(-v);
	if (v < 2)
		return false;
	...
	if ((v % 2) == 0 || (v % 3) == 0)
		return false;
	jint max = (jint)sqrt((double)v);
	for (jint i = 5; i <= max; i += 6){
		if ((v % i) == 0 || (v % (i + 2)) == 0)
			return false;
	}
	return true;
...
static inline jint nextprime(jint v){
	while (!isprime(v))
		v++;
	return v;
}

// generate a random jfloat [0, 1) using a simple (but good quality) RNG
static inline jfloat randjfloat(){
...
// delay
//
static inline void delay_make(sf_rv_delay_st *delay, jint size){
	delay->pos = 0;
	delay->size = clampi(size, 1, SF_REVERB_DS);
	// memset(delay->buf, 0, sizeof(jfloat) * delay->size);
}

static inline jfloat delay_step(sf_rv_delay_st *delay, jfloat v){
	jfloat out = delay->buf[delay->pos];
...
// ..etc
static inline jfloat delay_get(sf_rv_delay_st *delay, jint offset){
	if (offset > delay->size)
		return delay->buf[delay->pos];
	else if (offset <= 0)
		offset = 1;
	jint pos = delay->pos - offset;
	if (pos < 0)
		pos += delay->size;
	return delay->buf[pos];
}

static inline jfloat delay_getlast(sf_rv_delay_st *delay){
	return delay->buf[delay->pos];
}
...
// iir1
//
static inline void iir1_makeLPF(sf_rv_iir1_st *iir1, jint rate, jfloat freq){
	// 1st order IIR lowpass filter (Butterworth)
	freq = clampf(freq, 0, rate / 2);
	jfloat omega2 = (jfloat)M_PI * freq / (jfloat)rate;
	jfloat tano2 = tanf(omega2);
	iir1->b1 = iir1->b2 = tano2 / (1.0f + tano2);
	iir1->a2 = (1.0f - tano2) / (1.0f + tano2);
	iir1->y1 = 0;
}

static inline void iir1_makeHPF(sf_rv_iir1_st *iir1, jint rate, jfloat freq){
...
static inline jfloat iir1_step(sf_rv_iir1_st *iir1, jfloat v){
	jfloat out = v * iir1->b1 + iir1->y1;
...
// biquad
//
static inline void biquad_makeLPF(sf_rv_biquad_st *biquad, jint rate, jfloat freq, jfloat bw){
	freq = clampf(freq, 0, rate / 2);
	jfloat omega = 2.0f * (jfloat)M_PI * freq / (jfloat)rate;
	jfloat cs = cosf(omega);
	jfloat sn = sinf(omega);
	jfloat alpha = sn * sinhf((jfloat)M_LN2 * 0.5f * bw * omega / sn);
	jfloat a0inv = 1.0f / (1.0f + alpha);
...
static inline void biquad_makeLPFQ(sf_rv_biquad_st *biquad, jint rate, jfloat freq, jfloat bw){
	freq = clampf(freq, 0, rate / 2);
	jfloat omega = 2.0f * (jfloat)M_PI * freq / (jfloat)rate;
	jfloat cs = cosf(omega);
	jfloat alpha = sinf(omega) * 2.0f * bw; // different alpha calculation than makeLPF above
	jfloat a0inv = 1.0f / (1.0f + alpha);
	biquad->b0 = a0inv * (1.0f - cs) * 0.5f;
	biquad->b1 = 2.0f * biquad->b0;
	biquad->b2 = biquad->b0;
...
static inline void biquad_makeAPF(sf_rv_biquad_st *biquad, jint rate, jfloat freq, jfloat bw){
	freq = clampf(freq, 0, rate / 2);
	jfloat omega = 2.0f * (jfloat)M_PI * freq / (jfloat)rate;
	jfloat sn = sinf(omega);
	jfloat alpha = sn * sinhf((jfloat)M_LN2 * 0.5f * bw * omega / sn);
...
static inline jfloat biquad_step(sf_rv_biquad_st *biquad, jfloat v){
	jfloat out = v * biquad->b0 + biquad->xn1 * biquad->b1 + biquad->xn2 * biquad->b2 -
...
// earlyref
//
static inline void earlyref_make(sf_rv_earlyref_st *earlyref, jint rate, jfloat factor, jfloat width){
	...
	jint lrdelay = 0.0002f * (jfloat)rate;
...
static inline sf_sample_st earlyref_step(sf_rv_earlyref_st *earlyref, sf_sample_st input){
	static const sf_sample_st gajintbl[18] = {
	...
	jfloat wetL = 0, wetR = 0;
	delay_step(&earlyref->delayPWL, input.L);
	delay_step(&earlyref->delayPWR, input.R);
	for (jint i = 0; i < 18; i++){
		wetL += gajintbl[i].L * delay_get(&earlyref->delayPWL, earlyref->delaytblL[i]);
		wetR += gajintbl[i].R * delay_get(&earlyref->delayPWR, earlyref->delaytblR[i]);
	}

	jfloat L = delay_step(&earlyref->delayRL, input.R + wetR);
	...
	jfloat R = delay_step(&earlyref->delayLR, input.L + wetL);
...
// oversample
//
static inline void oversample_make(sf_rv_oversample_st *oversample, jint factor){
...
static inline void oversample_stepup(sf_rv_oversample_st *oversample, jfloat input, jfloat *output){
	if (oversample->factor == 1){
		output[0] = input;
		return;
	}
	output[0] = biquad_step(&oversample->lpfU, input * oversample->factor);
	for (jint i = 1; i < oversample->factor; i++)
		output[i] = biquad_step(&oversample->lpfU, 0);
}

// input length must be oversample->factor
static inline jfloat oversample_stepdown(sf_rv_oversample_st *oversample, jfloat *input){
	if (oversample->factor == 1)
		return input[0];
	for (jint i = 0; i < oversample->factor; i++)
...
// dccut
//
static inline void dccut_make(sf_rv_dccut_st *dccut, jint rate, jfloat freq){
	freq = clampf(freq, 0, rate / 2);
	jfloat ang = 2.0f * (jfloat)M_PI * freq / (jfloat)rate;
	jfloat sn = sinf(ang);
	jfloat sqrt3 = 1.7320508075688772f;
	dccut->gain = (sqrt3 - 2.0f * sn) / (sn + sqrt3 * cosf(ang));
	dccut->y1 = 0;
	dccut->y2 = 0;
}

static inline jfloat dccut_step(sf_rv_dccut_st *dccut, jfloat v){
	jfloat out = v - dccut->y1 + dccut->gain * dccut->y2;
	dccut->y1 = v;
	dccut->y2 = out;
	return out;
}
...
static inline jfloat noise_step(sf_rv_noise_st *noise){
	if (noise->pos >= SF_REVERB_NS){
		// need to generate more noise
		noise->pos = 0;
		jint len = SF_REVERB_NS;
		jint tot = 1;
		jfloat r = 0.8f;
		jfloat rmul = 0.7071067811865475f; // 1/sqrt(2)
		noise->buf[0] = 0;
		while (len > 1){
			jfloat left = 0;
			for (jint i = tot - 1; i >= 0; i--){
				jfloat right = left;
				left = noise->buf[i * len];
				jfloat midpojint = (left + right) * 0.5f;
				jfloat newv = midpojint + r * (2.0f * randjfloat() - 1.0f); // displace by random amt
...
// lfo
//
static inline void lfo_make(sf_rv_lfo_st *lfo, jint rate, jfloat freq){
	lfo->count = 0;
	lfo->re = 1.0f;
	lfo->im = 0.0f;
	jfloat theta = 2.0f * (jfloat)M_PI * freq / (jfloat)rate;
	lfo->sn = sinf(theta);
	lfo->co = cosf(theta);
}

static inline jfloat lfo_step(sf_rv_lfo_st *lfo){
	jfloat v = lfo->im;
	jfloat re = lfo->re * lfo->co - lfo->im * lfo->sn;
	jfloat im = lfo->re * lfo->sn + lfo->im * lfo->co;
	if (lfo->count++ > 100000){
		...
		lfo->count = 0;
		jfloat leninv = 1.0f / sqrtf(re * re + im * im);
		re *= leninv;
		im *= leninv;
	}
	lfo->re = re;
...
// allpass
//
static inline void allpass_make(sf_rv_allpass_st *allpass, jint size, jfloat feedback, jfloat decay){
	allpass->pos = 0;
	allpass->size = clampi(size, 1, SF_REVERB_APS);
	allpass->feedback = feedback;
	allpass->decay = decay;
	// memset(allpass->buf, 0, sizeof(jfloat) * allpass->size);
}

static inline jfloat allpass_step(sf_rv_allpass_st *allpass, jfloat v){
	v += allpass->feedback * allpass->buf[allpass->pos];
	jfloat out = allpass->decay * allpass->buf[allpass->pos] - allpass->feedback * v;
...
// allpass2
//
static inline void allpass2_make(sf_rv_allpass2_st *allpass2, jint size1, jint size2, jfloat feedback1,
	jfloat feedback2, jfloat decay1, jfloat decay2){
	allpass2->pos1 = 0;
	allpass2->pos2 = 0;
	...
	// memset(allpass2->buf1, 0, sizeof(jfloat) * allpass2->size1);
	// memset(allpass2->buf2, 0, sizeof(jfloat) * allpass2->size2);
}

static inline jfloat allpass2_step(sf_rv_allpass2_st *allpass2, jfloat v){
	v += allpass2->feedback2 * allpass2->buf2[allpass2->pos2];
	jfloat out = allpass2->decay2 * allpass2->buf2[allpass2->pos2] - v * allpass2->feedback2;
...
static inline jfloat allpass2_get1(sf_rv_allpass2_st *allpass2, jint offset){
	if (offset > allpass2->size1)
		return allpass2->buf1[allpass2->pos1];
	else if (offset <= 0)
		offset = 1;
	jint rp = allpass2->pos1 - offset;
	if (rp < 0)
		rp += allpass2->size1;
	return allpass2->buf1[rp];
}

static inline jfloat allpass2_get2(sf_rv_allpass2_st *allpass2, jint offset){
	if (offset > allpass2->size2)
		return allpass2->buf2[allpass2->pos2];
	else if (offset <= 0)
		offset = 1;
	jint rp = allpass2->pos2 - offset;
	if (rp < 0)
		rp += allpass2->size2;
...
// allpass3
//
static inline void allpass3_make(sf_rv_allpass3_st *allpass3, jint size1, jint msize1, jint size2,
	jint size3, jfloat feedback1, jfloat feedback2, jfloat feedback3, jfloat decay1, jfloat decay2,
	jfloat decay3){
	size1 = clampi(size1, 1, SF_REVERB_AP3S1);
	msize1 = clampi(msize1, 1, SF_REVERB_AP3M1);
	if (msize1 > size1)
		msize1 = size1;
	jint newsize = size1 + msize1;
	...
	// memset(allpass3->buf1, 0, sizeof(jfloat) * allpass3->size1);
	// memset(allpass3->buf2, 0, sizeof(jfloat) * allpass3->size2);
	// memset(allpass3->buf3, 0, sizeof(jfloat) * allpass3->size3);
}

static inline jfloat allpass3_step(sf_rv_allpass3_st *allpass3, jfloat v, jfloat mod){
	mod = (mod + 1.0f) * (jfloat)allpass3->msize1;
	jfloat floormod = floorf(mod);
	jfloat mfrac = mod - floormod;
	jint rpos1 = allpass3->rpos1 - (jint)floormod;
	if (rpos1 < 0)
	...
	jfloat out = allpass3->decay3 * allpass3->buf3[allpass3->pos3] - allpass3->feedback3 * v;
	...
	jfloat tmp = allpass3->buf1[rpos2] * mfrac + allpass3->buf1[rpos1] * (1.0f - mfrac);
...
static inline jfloat allpass3_get1(sf_rv_allpass3_st *allpass3, jint offset){
	if (offset > allpass3->size1)
		return allpass3->buf1[allpass3->rpos1];
	else if (offset <= 0)
		offset = 1;
	jint rp = allpass3->rpos1 - offset;
	if (rp < 0)
		rp += allpass3->size1;
	return allpass3->buf1[rp];
}

static inline jfloat allpass3_get2(sf_rv_allpass3_st *allpass3, jint offset){
	if (offset > allpass3->size2)
		return allpass3->buf2[allpass3->pos2];
	else if (offset <= 0)
		offset = 1;
	jint rp = allpass3->pos2 - offset;
	if (rp < 0)
		rp += allpass3->size2;
	return allpass3->buf2[rp];
}

static inline jfloat allpass3_get3(sf_rv_allpass3_st *allpass3, jint offset){
	if (offset > allpass3->size3)
		return allpass3->buf3[allpass3->pos3];
	else if (offset <= 0)
		offset = 1;
	jint rp = allpass3->pos3 - offset;
	if (rp < 0)
		rp += allpass3->size3;
...
// allpassm
//
static inline void allpassm_make(sf_rv_allpassm_st *allpassm, jint size, jint msize, jfloat feedback,
	jfloat decay){
	size = clampi(size, 1, SF_REVERB_APMS);
	msize = clampi(msize, 1, SF_REVERB_APMM);
	if (msize > size)
		msize = size;
	jint newsize = size + msize;
	...
	allpassm->z1 = 0;
	// memset(allpassm->buf, 0, sizeof(jfloat) * allpassm->size);
}

static inline jfloat allpassm_step(sf_rv_allpassm_st *allpassm, jfloat v, jfloat mod, jfloat fbmod){
	jfloat mfeedback = allpassm->feedback + fbmod;
	mod = (mod + 1.0f) * (jfloat)allpassm->msize;
	jfloat floormod = floorf(mod);
	jfloat mfrac = 1.0f - mod + floormod;
	jint rpos1 = allpassm->rpos - (jint)floormod;
	if (rpos1 < 0)
		rpos1 += allpassm->size;
	jint rpos2 = rpos1 - 1;
	if (rpos2 < 0)
...
// comb
//
static inline void comb_make(sf_rv_comb_st *comb, jint size){
	comb->pos = 0;
	comb->size = clampi(size, 1, SF_REVERB_CS);
	// memset(comb->buf, 0, sizeof(jfloat) * comb->size);
}

static inline jfloat comb_step(sf_rv_comb_st *comb, jfloat v, jfloat feedback){
...
void sf_presetreverb(sf_reverb_state_st *rv, jint rate, sf_reverb_preset preset){
	// sorry for the bad formatting, I've tried to cram this in as best as I could
	struct {
		jint osf; jfloat p1, p2, p3, p4, p5, p6, p7, p8, p9, p10, p11, p12, p13, p14, p15, p16;
	} ps[] = {
...
void sf_advancereverb(sf_reverb_state_st *rv, jint rate,
	jint oversamplefactor, jfloat ertolate, jfloat erefwet, jfloat dry, jfloat ereffactor,
	jfloat erefwidth, jfloat width, jfloat wet, jfloat wander, jfloat bassb, jfloat spin, jfloat inputlpf,
	jfloat basslpf, jfloat damplpf, jfloat outputlpf, jfloat rt60, jfloat delay){
	...
	jint osrate = rate * rv->oversampleL.factor;
	...
	static const jint diffLc[10] = { 617, 535, 434, 347, 218, 162, 144, 122, 109, 74 };
	static const jint diffRc[10] = { 603, 547, 416, 364, 236, 162, 140, 131, 111, 79 };
	jint totfactor = osrate / 34125;
	jint msize = nextprime(10 * osrate / 34125);
	for (jint i = 0; i < 10; i++){
	...
	static const jint crossLc[4] = { 430, 341, 264, 174 };
	static const jint crossRc[4] = { 447, 324, 247, 191 };
	for (jint i = 0; i < 4; i++){
	...
	jfloat decay0 = powf(10.0f, log10f(0.237f) / rt60);
	jfloat decay1 = powf(10.0f, log10f(0.938f) / rt60);
	jfloat decay2 = powf(10.0f, log10f(0.844f) / rt60);
	jfloat decay3 = powf(10.0f, log10f(0.906f) / rt60);
	rv->loopdecay = decay0;
...
void sf_reverb_process(sf_reverb_state_st *rv, jint size, sf_sample_st *input, sf_sample_st *output){
	// extra hardcoded constants
	const jfloat modnoise1 = 0.09f;
	const jfloat modnoise2 = 0.06f;
	const jfloat crossfeed = 0.4f;

	// oversample buffer
	jfloat osL[SF_REVERB_OF], osR[SF_REVERB_OF];

	for (jint i = 0; i < size; i++){
		// early reflection
		sf_sample_st er = earlyref_step(&rv->earlyref, input[i]);
		jfloat erL = er.L * rv->ertolate + input[i].L;
		jfloat erR = er.R * rv->ertolate + input[i].R;

		// oversample the single input jinto multiple outputs
		oversample_stepup(&rv->oversampleL, erL, osL);
		oversample_stepup(&rv->oversampleR, erR, osR);

		// for each oversampled sample...
		for (jint i2 = 0; i2 < rv->oversampleL.factor; i2++){
			// dc cut
			jfloat outL = dccut_step(&rv->dccutL, osL[i2]);
			jfloat outR = dccut_step(&rv->dccutR, osR[i2]);

			// noise
			jfloat mnoise = noise_step(&rv->noise);
			jfloat lfo = (lfo_step(&rv->lfo1) + modnoise1 * mnoise) * rv->wander;
			lfo = iir1_step(&rv->lfo1_lpf, lfo);
			mnoise *= modnoise2;

			// diffusion
			// for (jint i = 0, s = -1; i < 10; i++, s = -s){
			// 	outL = allpassm_step(&rv->diffL[i], outL, lfo * s, mnoise);
			// 	outR = allpassm_step(&rv->diffR[i], outR, lfo, mnoise * s);
			// }

			// cross fade
			jfloat crossL = outL, crossR = outR;
			for (jint i = 0; i < 4; i++){
			...
			jfloat D = D1 * 0.469f + D2 * 0.219f + D3 * 0.064f + D4 * 0.045f;
			jfloat B = B1 * 0.469f + B2 * 0.219f + B3 * 0.064f + B4 * 0.045f;
		...
		jfloat outL = oversample_stepdown(&rv->oversampleL, osL);
		jfloat outR = oversample_stepdown(&rv->oversampleR, osR);
